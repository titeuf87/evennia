...
function onText(args, kwargs) {
    ...
    if (renderto == "mainpane") {
        addTextToPane(args[0], renderto, kwargs);
    } else {
        openPopup(renderto, args[0]);
    }
}

function addTextToPane(text, panename, kwargs) {
    var mwin = $("div[data-name='"+panename+"'").find(".messagewindow");
    var cls = kwargs == null ? 'out' : kwargs['cls'];
    mwin.append("<div class='" + cls + "'>" + text + "</div>");
    mwin.parent().animate({
        scrollTop: mwin[0].scrollHeight
    }, 0);
...
function splitPane(parentpane, direction) {
        ...
        var splithtml = $("<div class='" + cls + "' data-size='50'></div>" +
                          "<div class='" + cls + "' data-size='50'>" + $("#dropdowntemplate").html() + '<div class="messagewindow" role="log"></div></div>');
    ...
    var createpanes = function(parentdiv, panes) {
        if ("direction" in panes) {
            ...
            if (parentdiv.is(':empty')) {
                ...
                parentdiv.append('<div class="messagewindow" role="log"></div>');
            };
        }
    }
